...
import components.PlanarTerranIF.AbstractPlanarTerran;
import components.TerranPlanar.PlanarTerranImpl;
...
public class TerranPlanarDefault extends Planar implements PlanarTerranIF {
	...
	protected static TerranPlanarDefault terranPlanar = new TerranPlanarDefault();
	protected static PlanarTerranImpl planarTerranImpl = terranPlanar.new PlanarTerranImpl();


	/**
	 * The documentation for all these invocations can be found in the
	 * ComplexPlanetOnly test.
	 * 
	 * 
	...
	public ImageCafe build(Boolean renderParameter) {
		...
		PlanarTerrainType planarTerrainType = new PlanarTerrainType(continent, terrain_type_tu_frequency,
				...
				terrain_type_def_mid_control_point_scalar, terrain_type_def_high_control_point, shelf_level,
				PlanarTerranIF.sea_level);
		...
		PlanarHighMountainType planarHighMountainType = new PlanarHighMountainType(mountainous_high_rm0_frequency,
				...
				NoiseQuality.QUALITY_BEST, mountainous_high_rm0_lacunarity, mountainous_high_tu_frequency,
				mountainous_high_tu_scalar1, mountainous_high_tu_scalar2, mountainous_high_tu_roughness,
				mountains_twist);
		...
		PlanarHillType planarHillType = new PlanarHillType(hilly_terrain_bi_frequency, inverse_res_in_meters,
				...
				inverse_res_in_meters, -1.0, inverse_res_in_meters, inverse_res_in_meters, inverse_res_in_meters,
				inverse_res_in_meters, inverse_res_in_meters, resinmeters, hilly_terrain_tu0_roughness,
				planet_circumference, meters_per_degree, lightcontrast, inverse_res_in_meters,
				hilly_terrain_tu1_roughness);
		...
		PlanarScaledHillTerrainType planarScaledHillTerrain = new PlanarScaledHillTerrainType(
				scaled_hilly_terrain_sb0_scale, scaled_hilly_terrain_sb0_bias, scaled_hilly_terrain_sb1_scale,
				scaled_hilly_terrain_sb1_bias, hillTerrain, scaled_hilly_terrain_pe_frequency,
				scaled_hilly_terrain_pe_persistence, hills_lacunarity, scaled_hilly_terrain_pe_octave_count,
				NoiseQuality.QUALITY_STD, scaled_hilly_terrain_ex_exponent);
		...
		PlanarContinentalShelfType planarContinentalShelfType = new PlanarContinentalShelfType(
				...
				continental_shelf_sb_scale, continental_shelf_sb_bias, continental_shelf_cl_bounds,
				PlanarTerranIF.sea_level);
		...
		PlanarContinentsWithPlainsType planarContinentsWithPlainsType = new PlanarContinentsWithPlainsType(
				baseContinentElevation, scaledPlainsTerrain);
		...
		PlanarContinentsWithMountainsType planarContinentsWithMountainsType = new PlanarContinentsWithMountainsType(
				...
				continent_with_mountains_edge_falloff, continentsWithHills, terrainType,
				PlanarTerranIF.mountains_amount);
		...
		PlanarFinalType planarFinalType = new PlanarFinalType(PlanarTerranIF.max_elev, PlanarTerranIF.min_elev,
				scalar_divisor, continentsWithRivers);
		...
		planet.build();
		RenderImageParameter renderImageParameter = new RenderImageParameter(PlanarTerranIF.gradientPointList, elevGrid,
				renderParameter, lightcontrast, lightcontrast);
